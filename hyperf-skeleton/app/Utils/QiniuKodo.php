...
class QiniuKodo
{
    ...
    public function uploadFile(string $key, string $localFilePath, bool $overwrite = true): array
    {
        ...
        // 执行上传操作
        [$ret, $err] = $uploadMgr->putFile($token, $key, $localFilePath);
        // 判断上传是否成功
        if ($err !== null) {
    ...
    public function uploadWithProgress(string $key, string $localFilePath): array
    {
        ...
        // 执行上传操作
        [$ret, $err] = $uploadMgr->putFile($token, $key, $localFilePath, null, null, 'application/octet-stream', $progress);
        // 判断上传是否成功
        if ($err !== null) {
            return ['status' => 'error', 'message' => $err->message()];
        }
    ...
    public function setLifecycle(string $key, int $days): array
    {
        ...
        // 执行持久化操作
        [$persistentId, $err] = $pfop->execute($this->bucketName, $key, $ops);
        if ($err !== null) {
            return ['status' => 'error', 'message' => $err->message()];
        }
